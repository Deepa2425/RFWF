...
public class TC_RFWF_SC_SS_19_OpenExperienceEditor extends BaseTestNGPageSiteCoreV2 {
	@Test(priority = 1)
	public void textTC_RFWF_SS_9_ConfigureVerticalComponent() throws InterruptedException {
		...
		String folderNameToSelect = g.getExcelCellValue(xlPath, sheetName, 3136, 2);
		String subFolder = g.getExcelCellValue(xlPath, sheetName, 3137, 2);
		String folderNameToCreate = g.getExcelCellValue(xlPath, sheetName, 3138, 2);

		String secure = g.getExcelCellValue(xlPath, sheetName, 3139, 2);
		String templateName = g.getExcelCellValue(xlPath, sheetName, 3140, 2);
		String tenantName = g.getExcelCellValue(xlPath, sheetName, 3141, 2);
		String subElement = g.getExcelCellValue(xlPath, sheetName, 3142, 2);
		String pageLayout = g.getExcelCellValue(xlPath, sheetName, 3143, 2);
...
cE.logout();
			...
			b = true;
			g.setExcelCellValue(xlPath, sheetName, 3136, 3, "PASS");
			g.setExcelCellValue(xlPath, sheetName, 3136, 4, "Executed on - " + date1);

		} catch (Exception e1) {
			g.setExcelCellValue(xlPath, sheetName, 3136, 3, "FAIL");
			g.setExcelCellValue(xlPath, sheetName, 3136, 4, date1);
		} finally {
			if (!b) {
				...
				g.setExcelCellValue(xlPath, sheetName, 3136, 3, "FAIL");
				g.setExcelCellValue(xlPath, sheetName, 3136, 4, "Executed on - " + date1);
				g.setExcelCellValue(xlPath, sheetName, 3136, 5, screenShot + v + "_" + date1);

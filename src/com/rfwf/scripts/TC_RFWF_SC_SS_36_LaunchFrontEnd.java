...
public class TC_RFWF_SC_SS_36_LaunchFrontEnd extends BaseTestNGPageSiteCoreV2 {
	@Test(priority = 1)
	public void textTC_RFWF_SC_SS_36_LaunchFrontEnd() throws InterruptedException {
		...
		String folderNameToSelect = g.getExcelCellValue(xlPath, sheetName, 3179, 2);
		String subFolder = g.getExcelCellValue(xlPath, sheetName, 3180, 2);
		String folderNameToCreate = g.getExcelCellValue(xlPath, sheetName, 3181, 2);

		String secure = g.getExcelCellValue(xlPath, sheetName, 3182, 2);
		String templateName = g.getExcelCellValue(xlPath, sheetName, 3183, 2);
		String tenantName = g.getExcelCellValue(xlPath, sheetName, 3184, 2);
		String subElement = g.getExcelCellValue(xlPath, sheetName, 3185, 2);
		String pageLayout = g.getExcelCellValue(xlPath, sheetName, 3186, 2);
		String frontEndUrlStrt = g.getExcelCellValue(xlPath, sheetName, 3187, 2);
		...
		try {
			 ...
			 * hP.clickSave1();
			 * 
			 * Thread.sleep(5000); iFT.switchFrame(); iFT.clickOK();
			 */ cE.publishOptions();
			cE.publishSite();
			iFT.switchFrame();
			...
			cE.getTitlePositive("slickslider");
			fE.verifyHeading("HighLight");

			b = true;

			// done
			b = true;
			g.setExcelCellValue(xlPath, sheetName, 3179, 3, "PASS");
			g.setExcelCellValue(xlPath, sheetName, 3179, 4, "Executed on - " + date1);

		} catch (Exception e1) {
			g.setExcelCellValue(xlPath, sheetName, 3179, 3, "FAIL");
			g.setExcelCellValue(xlPath, sheetName, 3179, 4, date1);
		} finally {
			if (!b) {
				...
				g.setExcelCellValue(xlPath, sheetName, 3179, 3, "FAIL");
				g.setExcelCellValue(xlPath, sheetName, 3179, 4, "Executed on - " + date1);
				g.setExcelCellValue(xlPath, sheetName, 3179, 5, screenShot + v + "_" + date1);

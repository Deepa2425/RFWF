...
public class TC_RFWF_SC_LOG_35_InsertForgotPasswordSameTenantInLogin extends BaseTestNGPageSiteCoreV2 {
	@Test(priority = 1)
	public void text_RFWF_SC_LOG_35_InsertForgotPasswordSameTenantInLogin() throws InterruptedException {
		...
		String folderNameToSelect = g.getExcelCellValue(xlPath, sheetName, 200, 2);
		String subFolder = g.getExcelCellValue(xlPath, sheetName, 201, 2);
		String folderNameToCreate = g.getExcelCellValue(xlPath, sheetName, 202, 2);
		String secure = g.getExcelCellValue(xlPath, sheetName, 203, 2);
		String templateName = g.getExcelCellValue(xlPath, sheetName, 204, 2);

		String tenantName = g.getExcelCellValue(xlPath, sheetName, 205, 2);
		String subElement = g.getExcelCellValue(xlPath, sheetName, 206, 2);
		String pageLayout = g.getExcelCellValue(xlPath, sheetName, 207, 2);
		String templateName1 = g.getExcelCellValue(xlPath, sheetName, 208, 2);
		String frontEndUrlStrt = g.getExcelCellValue(xlPath, sheetName, 209, 2);
		...
		try {
			...
			b = true;
			g.setExcelCellValue(xlPath, sheetName, 200, 3, "PASS");
			g.setExcelCellValue(xlPath, sheetName, 200, 4, "Executed on - " + date1);
		}

		catch (Exception e1) {
			// driver.close();
			g.setExcelCellValue(xlPath, sheetName, 200, 3, "FAIL");
			g.setExcelCellValue(xlPath, sheetName, 200, 4, date1);
		} finally {
			if (b == false) {
				...
				g.setExcelCellValue(xlPath, sheetName, 200, 3, "FAIL");
				g.setExcelCellValue(xlPath, sheetName, 200, 4, "Executed on - " + date1);
				g.setExcelCellValue(xlPath, sheetName, 200, 5, screenShot + v + "_" + date1);

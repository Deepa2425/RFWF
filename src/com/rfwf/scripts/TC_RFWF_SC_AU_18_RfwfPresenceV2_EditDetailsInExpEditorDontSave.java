...
public class TC_RFWF_SC_AU_18_RfwfPresenceV2_EditDetailsInExpEditorDontSave extends BaseTestNGPageSiteCoreV2 {
	@Test(priority = 1)
	public void textTC_RFWF_SC_REG_18_RfwfPresenceV2_EditDetailsInExpEditorDontSave() throws InterruptedException {
		...
		String folderNameToCreate = g.getExcelCellValue(xlPath, sheetName, 233, 2);
		...
		try {
			...
			b = true;
			g.setExcelCellValue(xlPath, sheetName, 231, 3, "PASS");
			g.setExcelCellValue(xlPath, sheetName, 231, 4, "Executed on - " + date1);

		} catch (Exception e1) {
			g.setExcelCellValue(xlPath, sheetName, 231, 3, "FAIL");
			g.setExcelCellValue(xlPath, sheetName, 231, 4, date1);
		} finally {
			if (!b) {
				...
				g.setExcelCellValue(xlPath, sheetName, 231, 3, "FAIL");
				g.setExcelCellValue(xlPath, sheetName, 231, 4, "Executed on - " + date1);
				g.setExcelCellValue(xlPath, sheetName, 231, 5,
